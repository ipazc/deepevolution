...
def fitness_func(model, X, Y):
    return model.evaluate(X, Y, batch_size=2048, verbose=0)[1]  # We return the accuracy as fitness score


# Evolve the model
history = model.fit_evolve(x_train, y_train, max_generations=10, population=10, top_k=3, fitness_func=fitness_func)
